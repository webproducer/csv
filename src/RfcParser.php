...
use CSV\Internal\{Lexer, Token};

use function CSV\Helpers\IO\makeReader;


class RfcParser implements ParserInterface
{
    ...
    public function parse($stream): \Iterator
    {
        ...
        foreach ($lexer->lex(makeReader($stream)) as [$type, $val, $pos]) {
...
//        );
    ...
    private function handleLineBreak(string $type, string $val, int $pos)
    {
        switch ($this->curState) {
            ...
            case self::S_NEXT_FLD:
            case self::S_UNQUOTED_FLD:
                if ($this->options->strict && ($this->curState !== self::S_CARRIAGE_RETURNED)) {
